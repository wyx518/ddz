...
public class Common {
	...
	public static CardType jugdeType(List<Card> list){
		...
		{//现在按相同数字最大出现次数
			...
			//飞机带n单,n/2对
			if(len>7&&card_index.a[2].size()==len/4 &&((card_index.a[0].size()==1&&card_index.a[1].size()==1)||(card_index.a[0].size()==len/4 ||2*card_index.a[1].size()==len/4))&&
					((Integer)(card_index.a[2].get(len/4-1))-(Integer)(card_index.a[2].get(0))==len/4-1))
				return CardType.c11122234;
			if(len==12&&card_index.a[2].size()==4&&(((Integer)(card_index.a[2].get(2))-(Integer)(card_index.a[2].get(0))==2)||((Integer)(card_index.a[2].get(3))-(Integer)(card_index.a[2].get(1))==2)))
				return CardType.c11122234;
			//飞机带n双
			if(len>9&&card_index.a[2].size()==len/5 && card_index.a[1].size()==len/5 &&len==card_index.a[2].size()*5&&
	...
	public static void getMax(Card_index card_index,List<Card> list){
		...
		for(int i=0,len=list.size();i<len;i++){
			...
			if(Common.getColor(list.get(i))==5)
				...
				{
				System.out.println("5");count[Common.getValue(list.get(i))-1]++;
	...
	public static int checkCards(List<Card> c,List<Card>[] current){
		...
		if(cType==CardType.c4)
		{
			if(c.size()==2)
				return 1;
			if(Common.jugdeType(currentlist)==CardType.c4&&currentlist.size()==2)
				return 0;
			if(Common.jugdeType(c)==Common.jugdeType(currentlist)&&Common.getValue(c.get(0))<=Common.getValue(currentlist.get(0)))
			{
				 return 0;
			}else {
				return 1;
